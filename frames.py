...
class Frames:
    ...
    def __init__(self, mainObj, MainWin, wWidth, wHeight, function, Object, xAxis=10, yAxis=10):
        ...
        self.btnClose.place(x=900, y=600)
    ...
    def NextWindow(self, methodToExecute):
        ...
        if (current == len(listWF) - 1):  #Trang 2, current = 1
            ...
            listWF[current].displayImage_1() #Hiện ảnh
            ...
            listWF[current + 1].displayImage_1()

        labelText = "ABC" if self.method == 1 else "DETECTION TUMOR"
        label = tkinter.Label(self.winFrame, text=labelText, width=24)
        label.configure(background="black", font=("Cambria", 20, "bold"), fg="pink")
        label.place(x=380, y=520)
    ...
    def displayImage_1(self):
        imgTk = self.image.resize((350, 350), Image.ANTIALIAS) #Chỉnh lại kích thước ảnh, sử dụng bộ lọc NTIALIAS
        imgTk = ImageTk.PhotoImage(image=imgTk) #mở ảnh sau khi xử lý
        self.image = imgTk #cho thuộc tính ảnh bằng ảnh imgTk
        self.labelImg = tkinter.Label(self.winFrame, image=self.image) #ẢNh hiện thị trong khung winFrame và ảnh hiển thị alf ảnh sau khi xử lý
        self.labelImg.place(x=400, y=150) #Chọn vị trí hiển thị ảnh
